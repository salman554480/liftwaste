                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <?php if ($email['status'] === 'assigned'): ?>
                                    <button class="btn btn-success w-100 mb-2" onclick="updateStatus(<?php echo $email_id; ?>, 'completed')">
    ...
    <script>
        ...
        function authenticateAndUpdateStatus(emailId, newStatus, email, password) {
            ...
            fetch('query/update_status.php', {
                ...
                body: 'email_id=' + emailId + '&status=' + newStatus + '&admin_email=' + encodeURIComponent(adminEmail)
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
            ...
            fetch('query/get_users.php')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('assignToUser').innerHTML = '<option value="">Failed to load users</option>';
                    }
                })
                .catch(error => {
